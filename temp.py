...
class Serve(object):
	...
	def __init__(self):
		...
		self.c=np.array([normal(15,0,6,22),-6.0,0]);
		self.d=np.array([np.random.normal(.5,0),np.random.normal(6.0,0),np.random.normal(98.0,0)]);
		self.p=self.c+self.d;

		self.rpm=normal(1500,0,1200,1600);
		q= np.array([np.random.normal(-.65,0),np.random.normal(-.25,0),np.random.normal(.35,0)])
		self.q = q/np.linalg.norm(q)

		u= np.array([np.random.normal(-.012,0),np.random.normal(.95,0),np.random.normal(-.04,0)]);
		self.v = np.random.normal(130,0)*17.6;
		...
		# print("u",self.u)
		# print('adrag',self.adrag);
		# print('agrav',self.agrav);
		# print('aspin',self.aspin);
		# print('rpm',self.rpm)
		# print("a", self.a);
		# print('p',self.p);
		# print('umag', self.v);
	...
	def landTime(self):
		...
		print('time', time_to_land((self.p[2]-self.rball),abs(self.u[2]),abs(self.a[2])));
...
if (Make_Plot):
	...
	plt.plot(servePositions[0],servePositions[1],"or", label = "Simulated Landings",markersize = 0.25);
